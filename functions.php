...
function universal_theme_widgets_init() {
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Сайдбар на странице поста', 'universal_theme' ),
			'id'            => 'sidebar-post',
			'description'   => esc_html__( 'Добавьте виджеты сюда', 'universal_theme' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
/**
 * Добавление нового виджета Collection_Widget (Виджет загрузчика).
 */
class Collection_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// __construct( $id_base, $name, $widget_options = array(), $control_options = array() )
		parent::__construct(
			'сollection_widget', // ID виджета, если не указать (оставить ''), то ID будет равен названию класса в нижнем регистре: сollection_widget
			'Доп.посты',
			array( 'description' => 'Сайдбар на странице поста', 'classname' => 'widget-сollection', )
		);

		// скрипты/стили виджета, только если он активен
		if ( is_active_widget( false, false, $this->id_base ) || is_customize_preview() ) {
			add_action('wp_enqueue_scripts', array( $this, 'add_сollection_widget_scripts' ));
			add_action('wp_head', array( $this, 'add_сollection_widget_style' ) );
		}
	}
    ...
    //вывод всего, что перед виджетом
		...
		if ( ! empty( $count ) ) {
			...
			echo '<div class="widget-сollection-wrapper">';
				global $post;
				$postslist = get_posts( array( 'posts_per_page' => $count, 'order'=> 'ASC', 'orderby' => 	'title' ) );
				foreach ( $postslist as $post ){
					setup_postdata($post);
	...
	// скрипт виджета
	function add_сollection_widget_scripts() {
		// фильтр чтобы можно было отключить скрипты
		if( ! apply_filters( 'show_my_widget_script', true, $this->id_base ) )
			return;
	...
	// стили виджета
	function add_сollection_widget_style() {
		// фильтр чтобы можно было отключить стили
		if( ! apply_filters( 'show_my_widget_style', true, $this->id_base ) )
			return;
		?>
...
// конец класса Collection_Widget

// регистрация Collection_Widget в WordPress
function register_сollection_widget() {
	register_widget( 'Collection_Widget' );
}
add_action( 'widgets_init', 'register_сollection_widget' );
//Завершили всю регистрацию Collection_Widget (Виджет - коллекция статей одной категории с конкретным постом)
